...
public class Player extends Mobile {
	...
	private Item[] items = {
			new Item(ItemIdentifiers.COINS, 10000),
			new Item(ItemIdentifiers.TINDERBOX),
			new Item(ItemIdentifiers.SMALL_FISHING_NET),
			new Item(ItemIdentifiers.BRONZE_AXE),
			new Item(ItemIdentifiers.BRONZE_PICKAXE),
			new Item(ItemIdentifiers.IRON_SCIMITAR),
			new Item(ItemIdentifiers.WOODEN_SHIELD),
			new Item(ItemIdentifiers.STAFF_OF_AIR),
			new Item(ItemIdentifiers.SHORTBOW),
			new Item(ItemIdentifiers.IRON_FULL_HELM),
			new Item(ItemIdentifiers.IRON_PLATEBODY),
			new Item(ItemIdentifiers.IRON_PLATELEGS),
			new Item(ItemIdentifiers.MIND_RUNE, 500),
			new Item(ItemIdentifiers.BRONZE_ARROW, 100),
			new Item(ItemIdentifiers.SWORDFISH_2, 10)
	};
	...
	public void onLogin() {
		...
		if (isNewPlayer()) {
			setNewPlayer(false);
			getInventory().addItems(items, true);
		}
		...
		getMembership().checkTime();
		if (getMembership().isMember()) {
			if (getDonatorRights().equals(DonatorRights.NONE)) {
				setDonatorRights(DonatorRights.REGULAR_DONATOR);
			}
		} else {
			setDonatorRights(DonatorRights.NONE);
		}
		getPacketSender().sendMessage("You currently have @red@" + getMembership().getDaysRemaining() + " @bla@days of membership remaining!");
		long totalExp = 0;
		for (Skill skill : Skill.values()) {
	...
	public void setSkillingPoints(int amount) {
		skillingPoints = amount;
	}
}
