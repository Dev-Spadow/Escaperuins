...
public class SkillManager {
	...
	private static final int EXPERIENCE_FOR_125 = 171077457;
	private static final int EXP_ARRAY[] = { 0, 83, 174, 276, 388, 512, 650, 801, 969, 1154, 1358, 1584, 1833, 2107,
			...
			11805606, 13034431, 14391160, 15889109, 17542976, 19368992, 21835073, 23611006, 26068632, 28782069, 31777943,
			35085654, 38737661, 42769801, 47221641, 52136869, 57563718, 63555443, 70170840, 77474828, 85539082, 94442737,
	...
	public static int getExperienceForLevel(int level) {
		if (level <= 125) {
			return EXP_ARRAY[--level > 124 ? 124 : level];
		} else {
			int points = 0;
			int output = 0;
	...
	public static int getLevelForExperience(int experience) {
		if (experience <= EXPERIENCE_FOR_125) {
			for (int j = 124; j >= 0; j--) {
	...
	public SkillManager addExperience(Skill skill, int experience, boolean multipliers) {
		// Multipliers...
		if (multipliers) {
			if (skill == Skill.ATTACK || skill == Skill.DEFENCE || skill == Skill.STRENGTH || skill == Skill.HITPOINTS || skill == Skill.RANGED || skill == Skill.MAGIC) {
				experience *= GameConstants.COMBAT_SKILLS_EXP_MULTIPLIER;
			} else {
				player.setSkillingPoints(player.getSkillingPoints() + experience);
