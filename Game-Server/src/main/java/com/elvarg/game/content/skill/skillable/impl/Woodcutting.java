...
public class Woodcutting extends DefaultSkillable {
    ...
    public void finishedCycle(Player player) {
        ...
        // Add ores..
        if (player.getEquipment().getItems()[Equipment.WEAPON_SLOT].getId() == 13241
                || player.getInventory().contains(13241) || player.getEquipment().getItems()[Equipment.WEAPON_SLOT].getId() == 22518
                || player.getInventory().contains(22518)) {
            if (Misc.inclusive(1, 3) == 3)
                infernalPerkTriggered = true;
        }
        if (infernalPerkTriggered) {
            player.getPacketSender().sendMessage("Your infernal axe burns hot.");
            player.getSkillManager().addExperience(Skill.FIREMAKING, Firemaking.LightableLog.getForItem(tree.getLogId()).get().getExperience());
        } else {
            player.getInventory().add(tree.getLogId(), 1);
            player.getPacketSender().sendMessage("You get some " + ItemDefinition.forId(tree.getLogId()).getName());
        }
    ...
    public static enum Axe {
        INFERNO_ADZE(22518, 1, 0.25, new Animation(2117)),
