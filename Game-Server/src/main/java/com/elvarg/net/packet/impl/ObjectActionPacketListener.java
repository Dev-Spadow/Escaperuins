...
import com.elvarg.game.content.christmas.CrateSearch;
...
import com.elvarg.game.entity.impl.object.impl.FirstHomePortal;
import com.elvarg.game.entity.impl.object.impl.SecondHomePortal;
import com.elvarg.game.entity.impl.object.impl.SpiritTreeNetwork;
...
public class ObjectActionPacketListener extends ObjectIdentifiers implements PacketExecutor {
    ...
    private static void firstClick(Player player, GameObject object) {
        ...
        if (object.getDefinition().getName().equalsIgnoreCase("spirit tree")) {
            player.getDialogueManager().start(new SpiritTreeNetwork(), 0);
            return;
        }
        if (CrateSearch.isValid(player, new Location(object.getLocation().getX(), object.getLocation().getY(), object.getLocation().getZ()), object.getDefinition().getName().toLowerCase()))
            return;

        switch (object.getId()) {
        case BANK_DEPOSIT_BOX_2:
            player.getBank(player.getCurrentBankTab()).open();
            /*
            player.getPacketSender().sendInterface(4465);
        ...
        case BANK_PORTAL:
            player.getDialogueManager().start(new FirstHomePortal(), 0);
        break;

        case DITCH_PORTAL:
            player.getDialogueManager().start(new SecondHomePortal(), 0);
            break;
