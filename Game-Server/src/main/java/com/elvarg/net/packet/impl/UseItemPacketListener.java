...
public class UseItemPacketListener extends ItemIdentifiers implements PacketExecutor {
    ...
    private static void itemOnObject(Player player, Packet packet) {
        ...
        if (object.getDefinition().getName().toLowerCase().contains("range") || object.getDefinition().getName().toLowerCase().contains("stove") || object.getDefinition().getName().toLowerCase().contains("fire")) {
            Optional<Cookable> cookable = Cookable.getForItem(item.getId());
            if (cookable.isPresent()) {
                player.getPacketSender().sendCreationMenu(new CreationMenu("How many would you like to cook?", Arrays.asList(cookable.get().getRawItem()), (productId, amount) -> {
