...
import com.elvarg.game.content.christmas.Box;
import com.elvarg.game.content.cluescrolls.ClueHandler;
...
public class ItemActionPacketListener implements PacketExecutor {
	...
	private static void firstAction(final Player player, Packet packet) {
		...
		if (player.getRights().isStaff()) {
			player.getPacketSender().sendMessage("[ItemActionPacket1] ItemId: " + itemId + " slot: " + slot + " InterfaceId: " + interfaceId);
		}
		...
		if (ClueHandler.sendRiddleToPlayer(player, itemId)) {
			return;
		}
		
		if (RewardHandler.open(player, itemId)) {
			return;
		}

		switch (itemId) {
		case ItemIdentifiers.MYSTERY_BOX:
			Box.openbox(player);
		break;
			...
			TaskManager.submit(new Task(1, player, false) {
				...
				protected void execute() {
					if (!player.isTeleporting()) {
						Barrows.dig(player);
						ClueHandler.handleRiddleDig(player);
					}
					stop();
				}
			});
		...
		case Gambling.MITHRIL_SEEDS:
			...
			break;
			case ItemIdentifiers.OLD_SCHOOL_BOND:
				if (!player.getInventory().contains(ItemIdentifiers.OLD_SCHOOL_BOND)) {
					player.getPacketSender().sendMessage("You need a bond to claim a bond!");
					return;
				}
				player.getMembership().addTime(14);
				player.getPacketSender().sendMessage("You have added 14 days of membership to your account!");
				player.getInventory().delete(new Item(ItemIdentifiers.OLD_SCHOOL_BOND, 1));
				player.getInventory().add(new Item(ItemIdentifiers.OLD_SCHOOL_BOND_UNTRADEABLE_, 1));
			break;
		case 9520:
	...
	public static void secondAction(Player player, Packet packet) {
		...
		if (player.getRights().isStaff()) {
			player.getPacketSender().sendMessage("[ItemActionPacket2] ItemId: " + itemId + " slot: " + slot + " InterfaceId: " + interfaceId);
		}
		if (slot < 0 || slot >= player.getInventory().capacity())
			return;
		if (player.getInventory().getItems()[slot].getId() != itemId)
	...
	public void thirdClickAction(Player player, Packet packet) {
		...
		int interfaceId = packet.readLEShortA();
		if (player.getRights().isStaff()) {
			player.getPacketSender().sendMessage("[ItemActionPacket3] ItemId: " + itemId + " slot: " + slot + " InterfaceId: " + interfaceId);
		}
		if (slot < 0 || slot >= player.getInventory().capacity())
			return;
		if (player.getInventory().getItems()[slot].getId() != itemId)
